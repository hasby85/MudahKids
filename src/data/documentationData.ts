...
CREATE TABLE IF NOT EXISTS users (
  ...
  membership_plan VARCHAR(20) DEFAULT 'PREMIUM',
  access_code VARCHAR(100) DEFAULT 'MudahKids2026',
  password TEXT,
  created_at TIMESTAMP WITH TIME ZONE DEFAULT NOW(),
  updated_at TIMESTAMP WITH TIME ZONE DEFAULT NOW()
);

-- 2. Synced User Data Table
...
-- 3. Master Key-Value Store Table (Fallback)
CREATE TABLE IF NOT EXISTS mudahkids_store (
  id VARCHAR(100) PRIMARY KEY DEFAULT 'master_store',
  data JSONB NOT NULL,
  updated_at TIMESTAMP WITH TIME ZONE DEFAULT NOW()
);
...
-- Disable RLS on these tables so Anon API Key can read/write freely without permission errors
ALTER TABLE users DISABLE ROW LEVEL SECURITY;
ALTER TABLE synced_data DISABLE ROW LEVEL SECURITY;
ALTER TABLE mudahkids_store DISABLE ROW LEVEL SECURITY;
