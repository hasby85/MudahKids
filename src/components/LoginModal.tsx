...
export const LoginModal: React.FC<LoginModalProps> = ({
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setLoading(true);
    try {
      const result = await loginAccount(email.trim(), password.trim());
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          <div>
            <label className="block text-xs font-extrabold text-stone-700 mb-1">
              {language === "en" ? "Registered Email or Phone *" : "Emel atau No. Telefon Berdaftar *"}
            </label>
            <div className="relative">
              <Mail className="w-4 h-4 text-stone-400 absolute left-3 top-3.5" />
              <input
                type="text"
                placeholder="nama@example.com / 017XXXXXXX / username"
                value={email}
          ...
          <button
            type="submit"
            disabled={loading}
            className="w-full py-3.5 mt-2 rounded-2xl bg-emerald-600 hover:bg-emerald-700 disabled:opacity-50 text-white font-extrabold text-sm shadow-md transition-all cursor-pointer flex items-center justify-center gap-2"
          >
            <LogIn className="w-4 h-4" />
            <span>{loading ? (language === "en" ? "Logging in..." : "Memuatkan...") : (language === "en" ? "Log In" : "Log Masuk")}</span>
          </button>
        </form>
